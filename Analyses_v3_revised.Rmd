...
# --- Add total row ---
total_row <- species_table_data %>%
  summarise(
    Species = "Total",
    Count_2005 = sum(Count_2005, na.rm = TRUE),
    Count_2024 = sum(Count_2024, na.rm = TRUE),
    Change_percent = round((Count_2024 - Count_2005) / Count_2005 * 100, 1),
    Rate_per_100k_2005 = sum(Rate_per_100k_2005, na.rm = TRUE),
    Rate_per_100k_2024 = sum(Rate_per_100k_2024, na.rm = TRUE),
    Rate_change_percent = round((Rate_per_100k_2024 - Rate_per_100k_2005) / Rate_per_100k_2005 * 100, 1)
  )

# Append total row
species_table_data <- bind_rows(species_table_data, total_row)
...
ggsave("figure_1.png", figure_1, width = 14, height = 20, scale = 0.67)
...
ggsave("figure_2.png", figure_2_all, width = 18, height = 14, scale = 0.67)
...
shannon_by_year <- df_species_counts %>%
  group_by(Year) %>%
  summarise(
    H = diversity(Count, index = "shannon"),              # Shannon index H'
    #S = sum(count > 0),                                   # Number of observed species categories
    #J = H / log(S),                                       # Pielou's evenness
    D1 = exp(H)                                           # Effective number of categories (Hill number)
  ) %>%
  ungroup()
...
dominance_by_year <- df_species_counts %>%
  group_by(Year) %>%
  mutate(
    Total = sum(Count),
    p = Count / Total
  ...
  summarise(
    Berger_Parker = max(p),                                    # Share of most common species
    Simpson_D     = sum(p^2),                                  # Simpson dominance
    Top3_share    = sum(sort(p, decreasing = TRUE)[1:min(3, n())]),
    Top5_share    = sum(sort(p, decreasing = TRUE)[1:min(5, n())]),
    N_species     = n(),                                       # Number of species categories
    .groups = "drop"
  )

# View results
print(dominance_by_year)
